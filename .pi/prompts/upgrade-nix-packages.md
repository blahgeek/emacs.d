...
帮我更新 Nix 目录下的版本。版本主要记录在 sources.json 中，是使用 niv 管理的。可能也有其他版本是直接在 home.nix 中记录的。

具体的更新流程如下：
0. 开始之前，先 `home-manager -f home.nix build` 一遍生成 result 目录，并将其重命名为 result.old
1. 使用 niv 来更新 sources.json 里每一个 repo 的版本。
  a. 注意，里面有一些包指定的是特定的 tag（例如 v1.1），我希望找到它最新的 tag 并更新过去，而不是直接切到 master。
  b. 对于其他没有指定 tag 而是指向 branch 的包，直接更新到该 branch 的最新commit即可。
2. 查看home.nix中单独指定了source有版本的包（如有），也按需更新版本
