...
**注意：下面提到的 tavily.sh 文件位于当前目录下，运行时请替换成绝对路径 `${current_skill_dir}/tavily.sh`**

通过 `./tavily.sh <endpoint> '<json-body>'` 调用 Tavily API，`<json-body>` 直接透传给 `https://api.tavily.com/<endpoint>`。

endpoint 选择：

- **search**：Web 搜索，输入 query 返回相关结果（可选 LLM answer）。
- **extract**：抓取给定 URL 的正文。
- **crawl**：从起点 URL 遍历站内页面并返回正文。
- **map**：同 crawl 的遍历，但只返回 URL 列表。
...
与 crawl 参数基本一致，但只返回 URL 列表，不抓正文，因此没有 `extract_depth` / `format` 等和抓取相关的参数。
